...
describe('Reducer', () => {
	it('should update the state correctly with UPDATE_NOTIFICATIONS', () => {
		let state;
		state = reducer(
			{
				notifications: [],
				modalState: { open: false },
				header: { hasNotification: false, numberOfNotifications: 0 },
			},
			{
				...
				payload: [
					...
					},
					{
						...
						title:
							'How to professionally administrate seamless growth strategies in 10 steps',
						likes: [
							{
							...
							{
								userId: '7bd3695eba3be49ef29bd423b12555bc',
						...
						comments: [
							{
								...
								name: 'Ali Sage',
								comment:
									'True! And after that they should functionalize core competencies',
							},
							{
	...
	it('should update the state correctly with HAS_NOTIFICATION', () => {
		let state;
		state = reducer(
			{
				notifications: [
			...
			{ type: 'HAS_NOTIFICATION', payload: { numberOfNotifications: 17 } }
		);
		expect(state).toEqual({
			notifications: [
				{
			...
			modalState: { open: false },
			header: { hasNotification: true, numberOfNotifications: 17 },
		});
	});

	it('should update the state correctly with TOOGLE_MODAL', () => {
		let state;
		state = reducer(
			{
				notifications: [
